...
def generate_video():
    camera = cv2.VideoCapture(0) # 0 corresponds to the default camera (webcam)
    detector = HandDetector(maxHands=2)  # Initialize hand detector
    offset = 20
    imgSize = 300
    while True:
        ...
        # Yield the frame as bytes in multipart response format
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + jpeg.tobytes() + b'\r\n\r\n')
...
# # Generator function to generate video stream with predictions
# def generate_video():
